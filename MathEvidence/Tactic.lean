...
import MathEvidence.Tactic.ReifyAnalytic
...
- Replay mode: load a committed bundle, run `RationalEquality.check`, close via
  `replaySound` / Bridge (`eq_of_proposition`) — not independent `field_simp; ring`.
