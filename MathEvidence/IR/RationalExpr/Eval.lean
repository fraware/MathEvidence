...
/-- Partial evaluation: `none` if a denominator is zero or a `rat` literal has `d = 0`. -/
def eval {α : Type*} [Field α] [DecidableEq α] (env : Env α) : Expr → Option α
  | .var i => some (env i)
  | .int n => some (n : α)
  | .rat n d =>
    if d = 0 then none
...
def Defined {α : Type*} [Field α] [DecidableEq α] (env : Env α) : Expr → Prop
  | .var _ => True
  | .int _ => True
  | .rat _ d => d ≠ 0
  | .neg e => Defined env e
...
noncomputable def evalDefined {α : Type*} [Field α] [DecidableEq α] (env : Env α) :
...
def conditionsHold {α : Type*} [Field α] [DecidableEq α] (env : Env α) (conds : List Expr) :
    Prop :=
  ∀ c ∈ conds, ∃ v, eval env c = some v ∧ v ≠ 0

/-- Executable check that every condition is defined and nonzero. -/
def conditionsHoldDec {α : Type*} [Field α] [DecidableEq α]
