...
OPERATIONS: dict[str, OperationDescriptor] = {
    ...
    "open_bundle": OperationDescriptor(
        id="open_bundle",
        summary="Open a Candidate Bundle; always returns computed (never Certified)",
        ...
        notes=[
            "Candidate Bundles report computed only.",
            "Verified status requires open_certification on a Certification Record.",
            "Returns opaque bundleId / digests — never filesystem paths.",
        ],
    ),
    "open_certification": OperationDescriptor(
        id="open_certification",
        summary="Verify a Certification Record; verified only after full digest coherence",
        input_schema=f"{_BASE}open-certification.input.schema.json",
        output_schema=_RESULT,
        claim_classes=["replay", "soundResult"],
        max_resource_policy={"maxWallTimeMs": 30_000, "maxOutputBytes": 1_048_576},
        notes=[
            "Requires real theorem identity + axiom report roles.",
            "native_checked must not report soundness_verified.",
            "Returns certificationId — never filesystem paths.",
        ],
    ),
    "replay_bundle": OperationDescriptor(
        id="replay_bundle",
        summary="Alias of verify_bundle — offline Candidate Bundle verification",
        input_schema=f"{_BASE}replay-bundle.input.schema.json",
        output_schema=_RESULT,
        claim_classes=["replay"],
        max_resource_policy={"maxWallTimeMs": 30_000, "maxOutputBytes": 1_048_576},
        notes=[
            "Operational verify-bundle / Python packaging only.",
            "Does not mutate the Candidate Bundle or emit Certification Records.",
            "Prefer verify_bundle; this id remains for compatibility.",
        ],
    ),
    "verify_bundle": OperationDescriptor(
        id="verify_bundle",
        summary="Operational Candidate Bundle verification (native_checked at most)",
        input_schema=f"{_BASE}replay-bundle.input.schema.json",
        output_schema=_RESULT,
        claim_classes=["replay"],
        max_resource_policy={"maxWallTimeMs": 30_000, "maxOutputBytes": 1_048_576},
        notes=[
            "mathevidence-verify-bundle / Python packaging only.",
            "Never emits theorem-level status or Certification Records.",
        ],
    ),
    "kernel_replay": OperationDescriptor(
        id="kernel_replay",
        summary="Theorem-producing kernel replay; emits Certification Record",
        input_schema=f"{_BASE}kernel-replay.input.schema.json",
        output_schema=_RESULT,
        claim_classes=["replay", "soundResult"],
        max_resource_policy={"maxWallTimeMs": 600_000, "maxOutputBytes": 4_194_304},
        notes=[
            "Applies replaySound / checker soundness; queries axioms.",
            "Returns certificationId only — never filesystem paths.",
            "Certified requires open_certification on the returned record.",
        ],
    ),
    "list_certifications_for_request": OperationDescriptor(
        id="list_certifications_for_request",
        summary="List Certification Record IDs bound to a request digest",
        input_schema=f"{_BASE}list-certifications.input.schema.json",
        output_schema=_RESULT,
        claim_classes=["discovery"],
        max_resource_policy={"maxWallTimeMs": 5_000, "maxOutputBytes": 65_536},
        notes=["Returns opaque certificationId values only."],
    ),
